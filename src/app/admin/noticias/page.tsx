...
async function getPosts() {
  return dbQuery<PostRow[]>(
    "SELECT id, title, summary, tag, cover, cover_label, published_at, sort_order, active FROM site_posts WHERE type = 'news' ORDER BY published_at DESC, sort_order ASC, id DESC",
  );
}
...
async function createPost(formData: FormData) {
  ...
  await dbQuery(
    `INSERT INTO site_posts
      (type, title, summary, tag, cover, cover_label, published_at, sort_order, active, created_at, updated_at)
     VALUES
      ('news', :title, :summary, :tag, :cover, :cover_label, :published_at, :sort_order, :active, NOW(), NOW())`,
    {
      title,
      summary,
      tag,
  ...
  return (
    <div className="admin-page">
      <header className="admin-header">
        <h1>Notícias</h1>
        <p>Gerencie os cards de notícias exibidos na home e na página de notícias.</p>
      </header>

      <section className="admin-split">
        <div className="card admin-card">
          <h2 className="card-title">Nova notícia</h2>
          <form className="admin-form" action={createPost}>
            <label>
              Título
              <input name="title" placeholder="Temporada 5 liberada" />
            </label>
            <label>
              Resumo
            ...
            <label>
              Tag
              <input name="tag" placeholder="S5" />
            </label>
            <label>
              Visual (vip, crates, cosmetics)
              <input name="cover" placeholder="vip" />
            </label>
            <label>
        ...
        <div className="card admin-card">
          <h2 className="card-title">Notícias cadastradas</h2>
          <div className="admin-list">
            ...
            {!posts.length && <p className="muted">Nenhuma notícia cadastrada.</p>}
          </div>
        </div>
      </section>
    </div>
